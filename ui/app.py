...
class ProductScraperApp:
    ...
    def create_widgets(self):
        ...
        # Treeview oluştur - resim sütunu ekle
        self.tree = ttk.Treeview(middle_frame, columns=("image", "title", "price", "url"), show="headings")
        self.tree.heading("image", text="Resim")
        ...
        # İstatistik düğmesi ekle
        self.stats_button = ttk.Button(button_frame, text="Tarama İstatistikleri", command=self.show_stats)
    ...
    def show_product_preview(self, product, index):
        ...
        def on_save(updated_product):
            ...
            self.tree.item(str(index), values=(
                "Resim" if updated_product["image_url"] else "",
                updated_product["title"],
                updated_product["price"],
                updated_product["product_url"]
            ))
            
            # Eğer resim varsa, resmi güncelle
            if updated_product["image_url"]:
                photo = self.load_product_image(updated_product["image_url"])
                if photo:
                    self.tree.item(str(index), image=photo)
    ...
    def load_product_image(self, image_url, size=(50, 50)):
        """Ürün resmini yükler ve önbelleğe alır."""
        if not image_url:
            return None
            
        # Eğer resim zaten önbellekteyse, onu kullan
        if image_url in self.image_cache:
            return self.image_cache[image_url]
            
        try:
            response = requests.get(image_url, verify=self.scraper.verify_ssl, timeout=5)
    ...
    def update_product_list(self, products):
        ...
        for i, product in enumerate(products):
            # Resmi yükle
            photo = self.load_product_image(product["image_url"])
            
            # Ürünü ekle
            item = self.tree.insert("", tk.END, iid=i, values=(
                "",  # Resim hücresi başlangıçta boş
                product["title"],
                product["price"],
                product["product_url"]
            ))
            
            # Eğer resim yüklendiyse, resmi hücreye ekle
            if photo:
                self.tree.item(item, image=photo, values=(
                    "Resim",  # Resim hücresi için metin
                    product["title"],
                    product["price"],
                    product["product_url"]
                ))
    ...
    def show_stats(self):
        ...
        if not hasattr(self.scraper, 'stats'):
            messagebox.showinfo("Bilgi", "Henüz tarama yapılmadı.")
            return
        ...
        stats_text.config(state=tk.DISABLED)  # Salt okunur yap
